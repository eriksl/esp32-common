...
namespace Crypt
{
	std::string Generic::password_to_aes256_key(std::string_view password)
	{
		std::string salted_password;

		salted_password.assign(reinterpret_cast<const char *>(Generic::password_salt), sizeof(Generic::password_salt));
		salted_password.append(password);
		return(sha256(salted_password));
	}

	std::string Generic::hash_to_text(std::string_view hash)
	{
		char raw_value;
	...
	uint32_t Generic::crc32(std::string_view in)
	{
		Crypt::CRC32 crc;

		crc.init();
		crc.update(in);
		return(crc.string_to_uint32(crc.finish()));
	}

	std::string Generic::sha256(std::string_view in)
	{
		Crypt::SHA256 sha;
	...
	std::string Generic::aes256(bool encrypt, std::string_view key, std::string_view in)
	{
		Crypt::AES256 aes;
		std::string out;
	...
	const uint32_t CRC32::crc32_table[] =
	{
	...
	std::string CRC32::uint32_to_string(uint32_t in)
	{
		std::string out;
	...
	uint32_t CRC32::string_to_uint32(std::string_view in)
	{
		uint32_t out = 0;
	...
	void CRC32::init(bool, std::string_view initial)
	{
		...
		if(!initial.empty())
		{
			...
			this->crc = this->string_to_uint32(initial);
		}
		else
	...
	std::string CRC32::update(std::string_view data)
	{
		...
		return(this->uint32_to_string(this->crc));
	}
